...
locals {
  ...
  config_audit_role    = var.allow_config_metadata_audit ? ["arn:aws:iam::${data.aws_caller_identity.current.id}:role/aws-service-role/config.amazonaws.com/AWSServiceRoleForConfig"] : []
  access_analyzer_role = var.allow_access_analyzer_metadata_audit ? ["arn:aws:iam::${data.aws_caller_identity.current.id}:role/aws-service-role/access-analyzer.amazonaws.com/AWSServiceRoleForAccessAnalyzer"] : []
